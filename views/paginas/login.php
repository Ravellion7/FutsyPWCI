...
if (!isset($exito) || !is_string($exito)) {
    $exito = '';
}
...
<div class="absolute top-[20px] sm:top-[40px] md:top-0 left-[5px] sm:left-[20px] md:left-10 overflow-hidden h-[610px] sm:h-[390px] md:h-[710px] z-0">
    ...
    <div class="bg-[#9FBE4E] px-4 sm:px-1 md:px-10 lg:px-12 py-4 sm:py-1 md:py-10 lg:py-12 rounded-xl shadow-2xl w-[65%] max-w-[420px] md:max-w-[500px] lg:max-w-[550px] xl:w-[36%] flex flex-col justify-between border border-white/20 hover:shadow-[0_25px_50px_-12px_rgba(164,195,81,0.4)] transition-shadow duration-300 mt-12 sm:mt-20 md:mt-0 ml-auto mr-1 sm:mr-0 md:mr-0 relative z-20">
        ...
        <div>
            ...
            <h3 class="text-white text-xl sm:text-2xl md:text-3xl font-bold text-center mb-2 sm:mb-3 md:mb-5 drop-shadow-md">Inicia sesión</h3>

            <form class="flex flex-col gap-1.5 sm:gap-2 md:gap-3" action="/login" method="post">
                ...
                <div class="flex flex-col gap-0.5 sm:gap-1 md:gap-2">
                    <div class="flex justify-between items-center">
                        ...
                        <a href="/forgot-password" class="text-white text-[10px] sm:text-xs hover:underline">¿Has olvidado tu contraseña?</a>
                    </div>
                    <input class="p-1.5 sm:p-2 md:p-2.5 lg:p-3 rounded-md border-2 border-transparent outline-none text-gray-600 text-[11px] sm:text-xs md:text-sm focus:outline-none focus:ring-2 focus:ring-[#BF7D24] focus:border-[#BF7D24] hover:border-[#BF7D24]/50 transition-all duration-200 shadow-sm" type="password" id="password" name="password" placeholder="Contraseña">
                </div>

                <div class="mt-1.5 sm:mt-2 md:mt-3 lg:mt-4 flex justify-center gap-2 sm:gap-3">
                    <button id="ingresarBtn" type="submit" class="bg-[#CE8F3A] text-white font-bold py-1.5 sm:py-2 md:py-2.5 lg:py-3 px-5 sm:px-6 md:px-7 lg:px-8 rounded-full hover:bg-[#D99A45] hover:shadow-lg hover:shadow-[#CE8F3A]/40 transition-all duration-200 text-[11px] sm:text-xs md:text-sm lg:text-base transform hover:scale-105 cursor-pointer active:scale-95">Ingresar</button>
                </div>
            </form>
        </div>
        ...
        <div class="mt-4 sm:mt-6 md:mt-8">
            ...
            <p class="text-white text-center text-[11px] sm:text-xs md:text-sm">¿Si aún no tienes cuenta? <a href="/register" class="underline hover:text-white/80">Regístrate</a></p>
        </div>
    </div>
...
<script>
    Swal.fire({
        ...
        window.location.href = <?php echo json_encode($redirectUrl ?: '/home'); ?>;
    });
</script>
<?php endif; ?>